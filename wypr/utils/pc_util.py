...
def write_ply_rgb(points, colors, filename, num_classes=None):
...
def write_obj_rgb(points, colors, filename, num_classes=None):
...
def points_within_boxes(point_cloud, boxes):
    # point_cloud[:, :3]: xyz
    # boxes[:, :6]: xyzlwh
    x_min =  boxes[:, 0] -  0.5 * boxes[:, 3]
    x_max =  boxes[:, 0] +  0.5 * boxes[:, 3]
    y_min =  boxes[:, 1] -  0.5 * boxes[:, 4]
    y_max =  boxes[:, 1] +  0.5 * boxes[:, 4]
    z_min =  boxes[:, 2] -  0.5 * boxes[:, 5]
    z_max =  boxes[:, 2] +  0.5 * boxes[:, 5]
    points_x = point_cloud[:, 0][:, None] 
    points_y = point_cloud[:, 1][:, None] 
    points_z = point_cloud[:, 2][:, None] 
    return  (points_x > x_min) & (points_x < x_max) & \
            (points_y > y_min) & (points_y < y_max) & \
